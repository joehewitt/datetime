...
exports.formatIn = function(d, timezone, baseTime, locale) {
    var localeData = getLocale(locale);
    if (!baseTime) {
        baseTime = new Date();
    }

    var delta = d - baseTime;
    if (delta < minute) {
        var n = Math.round(delta/second);
        var format = n == 1 ? localeData.secondIn : localeData.secondsIn;
        return format.replace('%t', n);
    } else if (delta < hour) {
        var n = Math.round(delta/minute);
        var format = n == 1 ? localeData.minuteIn : localeData.minutesIn;
        return format.replace('%t', n);
    } else if (delta < day) {
        var n = Math.round(delta/hour);
        var format = n == 1 ? localeData.hourIn : localeData.hoursIn;
        return format.replace('%t', n);
    } else if (delta < week) {
        return exports.format(d, '%A at %i:%M%P', timezone, locale);
    } else if (d.getYear() == baseTime.getYear()) {
